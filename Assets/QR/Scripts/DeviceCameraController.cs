...
public class DeviceCameraController : MonoBehaviour {
	...
	public void assignCameraPlane(GameObject cp) {
		e_CameraPlaneObj = cp;
		StartWork ();
	}
		...
		if (webcam != null && webcam.isPlaying()) {
			...
			if (e_CameraPlaneObj != null) {
				if (e_CameraPlaneObj.activeSelf) {
					e_CameraPlaneObj.GetComponent<RawImage>().texture = webcam.preview;
				}
			}
	...
	public void StopWork()
	{
		...
		if(e_CameraPlaneObj.activeSelf)
		{
			e_CameraPlaneObj.GetComponent<RawImage>().texture = null;
		}
