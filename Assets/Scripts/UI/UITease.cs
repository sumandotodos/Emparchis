...
public class UITease : MonoBehaviour {
	...
	void Update () {
		...
		SXSpeed += SXAccel * fixedDeltaTime;
		SYSpeed += SYAccel * fixedDeltaTime;

		SX += SXSpeed * fixedDeltaTime;
		SY += SYSpeed * fixedDeltaTime;

		Vector3 sc = new Vector3 (SX, SY, 1);
		this.transform.localScale = sc;

		if (burstTimeRemaining > 0.0f) {
			burstTimeRemaining -= fixedDeltaTime;
			if (burstTimeRemaining <= 0.0f) {
				targetSX = 1.0f;
				targetSY = 1.0f;
			}
