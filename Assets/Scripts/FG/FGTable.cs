...
public class FGTable : MonoBehaviour {
	...
	public object getElement(string col, int r) {
		for (int i = 0; i < column.Count; ++i) {
			if (column [i].columnName.Equals (col)) {
	...
	public string JSON2CRSV(string json)
	{
		...
		for (int i = 0; i < files.Length; ++i) {
			string[] fields = files [i].TrimEnd(' ').TrimEnd(',').Split (':');
	...
	public string exportCRSV(bool humanReadable) {
	...
	{		
		...
		for (int k = 0; k < nColumns; ++k) { // process each column
			...
			if (int.TryParse (files [offset + k], out intTest)) {
				...
				if (colNames == null || (colNames.Length == null)) {
			...
			for (int i = offset + k; i < files.Length; i += nColumns) {
				...
				//if (!files [i].Equals ("")) {
					if (type == FGTable.TypeInteger) {
						...
						rosetta.registerString (this.name + "_" + k + "_" + nRow, files [i]);
						++nRow;
					}
				//}
			}
