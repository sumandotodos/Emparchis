...
public class Type2TestController : Task {
	...
	public void startType2Test(Task w, Situation s) {
		...
		gameController.currentPlayerRole = getRandomFamily ();
		changedRole = TestAux.getRoleChangeMember (gameController.currentPlayerRole, s.table);


		((CommonTestController)waiter).destFamilyMember = changedRole;
		...
		((CommonTestController)waiter).destFamilyMember.mood = Mood.sad;
