...
public class Score {
	...
	public int shoesScore;
	//public int toOthersScore;
	//public int fromOthersScore;
	...
	public Score() {
		firstToGoal = shoesScore = pointsScore = total = hitos = 0;
	}

}

public class StatisticsController : Task {
	...
	void Start() {
		score = new Score[GameController.MaxPlayers];
	...
	public void calculateScores() {
		...
		for (int i = 0; i < GameController.MaxPlayers; ++i) {
			if (!gameController.playerPresent [i]) {
				for (int j = 0; j < TicketScreenController.MaxColors; ++j) {
...
//		totalBottlesText.text = "" + totalBottles;
		...
		int[] receivedBottles = new int[GameController.MaxPlayers];
		int[] receivedBottlesOrder = new int[GameController.MaxPlayers];
		for (int i = 0; i < GameController.MaxPlayers; ++i) {
			receivedBottles [i] = gameController.playerList [gameController.localPlayerN].bottlesReceived [i];
		}

		int[] givenBottles = new int[GameController.MaxPlayers];
		int[] givenBottlesOrder = new int[GameController.MaxPlayers];
		for (int i = 0; i < GameController.MaxPlayers; ++i) {
			givenBottles [i] = 0;
			for (int j = 0; j < TicketScreenController.MaxColors; ++j) {
...
//					gameController.playerList [i].bottlesReceived [TicketScreenController.ClearBottle] += (6/secondNPlayers);
...
//		}
		...
		int totalRedBottles = gameController.playerList [gameController.localPlayerN].bottlesReceived [TicketScreenController.RedBottle];
		int totalYellowBottles = gameController.playerList [gameController.localPlayerN].bottlesReceived [TicketScreenController.YellowBottle];
		int totalBlueBottles = gameController.playerList [gameController.localPlayerN].bottlesReceived [TicketScreenController.BlueBottle];
		int totalClearBottles = gameController.playerList [gameController.localPlayerN].bottlesReceived [TicketScreenController.ClearBottle];
